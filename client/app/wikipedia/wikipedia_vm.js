...
var vm = this;
    ...
    vm.MainData = {};
    vm.MainData.results = [];
    vm.MainData.listen = null;
    vm.MainData.search = function(term) {
      GetResults($('input.dirty').val());
    };

    // on random search call this
    vm.MainData.Random = function(term) {
      RandomSearch(term);
    };

    //When Mic icon is clicked
    vm.MainData.Speech = function(url) {
      StartSpeech();
    };
    ...
    function Render(data) {
      ...
      vm.MainData.results = []; // reset data
    ...
    function StartSpeech() {
      $('input.dirty').val(''); //clear input field
      if (vm.MainData.listen) {
        vm.MainData.listen = null;
        animate();
        return annyang.abort();
      } else {
        animate();
        vm.MainData.listen = true;
        var commands = {
          'find *term': function(term) {
            GetResults(term);
            vm.MainData.listen = null;
            vm.MainData.val = term;
            animate();
            findAudio.play();
            return annyang.abort();
